...
### OPCION ELEGIR LISTA DE PERSONAJES
# Seguir el flujo de la lista
## COMIENZA EL FLUJO
...
while opcion==True:
  ...
  if opcionLeer=="0":
    ...
    if re.search('^([\dA-ZÑÁÉÍÓÚa-zñáéíóú]+[-]?[\.]?[\s]?)+$',personajeNuevo):
      listas=agregarNombre(listas,index,personajeNuevo)
      print(listas[index])
    ...
    while True:
      ...
      if opcionsalir=="n" or opcionsalir=="no":
        break
    listas=agregarDiccionario(listas,serie,nombres)
